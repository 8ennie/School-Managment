...
export class EditShowPanelComponent implements OnInit, OnChanges {
  ...
  ngOnChanges(changes: SimpleChanges): void {
    if (changes.imageShowResourceUrl) {
      if (this.imageShowResourceUrl) {
        this.imageShowService.getImageShow(this.imageShowResourceUrl).then((imageShow: ImageShow) => {
          this.imageShow = imageShow;
          this.showPartService.getShowPartsFromImageShow(imageShow.resourceUrl).then((showParts: ShowPart[]) => {
            if (showParts) {
              this.imageShow.showParts = showParts.sort((n1, n2) => n1.position - n2.position);
            }
          });
        });
      } else {
  ...
  public save(): void {
    if (this.imageShowResourceUrl) {
      ...
      this.imageShowService.updateImageShow(this.imageShow).then((imageShow: ImageShow) => {
        this.showPartService.updateShowPartsForImageShow(showParts, imageShow.resourceUrl).then(
          (_) => {
            this.imageShow = imageShow;
      ...
      const showParts = this.imageShow.showParts;
      this.imageShow.showParts = null;
      this.imageShowService.saveImageShow(this.imageShow).then((imageShow: ImageShow) => {
        this.showPartService.saveShowPartsForImageShow(showParts, imageShow.resourceUrl).then(
          (_) => {
            this.imageShow = imageShow;
